#include "stdafx.h"
#include "Wraith.h"
Wraith::Wraith(float x, float y, Texture& textureSheet, EnemySpawner& enemySpawner, Entity& player)
	:Enemy(enemySpawner)
{
	this->InitVal();
	this->InitGui();
...
void Wraith::InitComponents()
{
}
void Wraith::InitAnimation()
{
	//add animation
...
void Wraith::UpdateAnimation(const float& dt)
{
	/*this->UpdateAttack();
	if (this->attacking) {
...
void Wraith::Update(const float& deltaTime, Vector2f& mousePosView, const View& view)
{
...
void Wraith::Render(RenderTarget& target, Shader* shader, const bool showHitBox)
{
